...
export default function NewRequisitionPage() {
  ...
  async function handleSave(mode: "draft" | "approval" | "direct_publish") {
    setSaving(true);
    try {
      const isDirectPublish = mode === "direct_publish";
      ...
      const payload = {
        ...
        shadowModeRequired,
        saveAsDraft: isDraft,
        publishDirect: isDirectPublish,
        status: isDraft ? "draft" : isDirectPublish ? "active" : "pending_approval",
      };
      ...
      if (res.ok) {
        if (isDirectPublish) {
          setSuccessMessage("⚡ Requisition published directly! Status is now ACTIVE and live on the public job board.");
        } else if (isDraft) {
          setSuccessMessage("Requisition saved as draft.");
        } else {
          setSuccessMessage(`✓ Requisition submitted for approval! Routed to ${hiringManager || "Hiring Manager"}.`);
        }
        setTimeout(() => {
      ...
      <header className="px-6 py-3.5 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3.5 py-1.5 text-xs font-semibold rounded-lg border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors flex items-center gap-1.5 disabled:opacity-50"
            title="Submit to Hiring Manager for review and approval"
          >
            <span>📨</span>
            <span>Submit for HM Approval</span>
          </button>
          <button
            onClick={() => handleSave("direct_publish")}
            disabled={saving || !title.trim()}
            className="px-4 py-1.5 text-xs font-bold rounded-lg bg-brand hover:bg-brand-dark text-white shadow-button transition-colors flex items-center gap-1.5 disabled:opacity-50"
            title="Bypass approval and publish immediately to candidate search and public career board"
          >
            <span>⚡</span>
            <span>{saving ? "Publishing..." : "Skip Approval & Publish Directly"}</span>
          </button>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-2xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  Designated Hiring Manager (for Approval &amp; Reviews) *
                </label>
                <input
                  type="text"
                  value={hiringManager}
